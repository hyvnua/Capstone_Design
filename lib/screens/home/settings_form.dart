...
class _SettingsFormState extends State<SettingsForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<member_UserData>(// stream을 이용해서 firestore 데이터 가져오기
      ...
      stream: DatabaseService(uid: user.uid).member_userData,//DatabaseService 위젯의 stream get을 이용해서 해당 uid의 name, sugar, strength가져옴
            ...
            child: Column(
              children: <Widget>[
                ...
                DropdownButtonFormField(
                  ...
                  value: _currentAge ?? member_userData!.age,//firestore로부터 가져온 데이터 중 sugars를 현재 sugar에다가 넣어줌

                  items: age.map((age) {//위에 있는 배열을 가져옴
                  //items: sugars.map((sugar) {//위에 있는 배열을 가져옴
                    ...
                    return DropdownMenuItem(//드롭다운형식으로 보여줌
                      ...
                      child: Text('$age Age'),//배열에 들어있는 값으로 차례로 드롭다운 메뉴형식으로 보여줌
                    );
                  }).toList(),// map을 사용했으니 tolist로 마무리
                  //onChanged: (value) => setState(() => _currentSugars = value.toString()),//변경된 값을 저장
                  onChanged: (value) => setState(() => _currentAge = value.toString()),//변경된 값을 저장
